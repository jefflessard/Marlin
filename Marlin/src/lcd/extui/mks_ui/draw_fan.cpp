...
static void event_handler(lv_event_t *event) {
  ...
  if (mks_data(event).mks_obj_id != ID_F_RETURN) disp_fan_value();
}
...
void lv_draw_fan() {
  ...
  lv_obj_add_style(fanText,&tft_style_label_rel,0);
  disp_fan_value();
}
...
void disp_fan_value() {
  ...
  lv_obj_align_to(fanText, nullptr, LV_ALIGN_CENTER, 0, -65);
}
